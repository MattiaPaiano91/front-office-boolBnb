  ...
  methods: {
    ...
    async getData() {
      try {
        const res = await axios.get(
          `http://localhost:8000/api/apartments/${this.$route.params.slug}`
        );
    ...
    closeModal() {
     ...
     initMap() {
      if (
        this.apartment &&
        this.apartment.longitude !== null &&
        this.apartment.latitude !== null
      ) {
        
        const lat = !isNaN(parseFloat(this.apartment.latitude)) ? parseFloat(this.apartment.latitude) : 0;
        const lng = !isNaN(parseFloat(this.apartment.longitude)) ? parseFloat(this.apartment.longitude) : 0;
      
        this.mapCenter = [lng !== null ? lng : 0, lat!== null ? lat : 0];
          
        const map =  tt.map({
          key: "H56lQ8whGOWJSY4sdjASyYRcj8aE4T1z",
          container: "map",
        ...
        map.scrollZoom.enable({ around: 'center' });
         ...
         map.on("load", () => {
          new tt.Marker().setLngLat(this.mapCenter).addTo(map);
        });
      }
    },
    scrollToTop() {
      ...
      <div v-if="apartment" class="row">
        ...
        <div class="col-12 col-sm-6 px-5">
          <ul class="p-0" id="apartment-details">
            ...
            <li>L'alloggio si trova in {{ apartment.address }}</li>
            <li>
            ...
            <li class="mt-4">
              <div>
                ...
                <ul class="p-0">
                  <li>Numero di ospiti: {{ apartment.n_guests }}</li>
                  <li>Numero di stanze: {{ apartment.n_rooms }}</li>
                  <li>Numero di letti: {{ apartment.n_beds }}</li>
                  <li>Numero di bagni: {{ apartment.n_baths }}</li>
                  <li>Metratura {{ apartment.mq }} m²</li>
                </ul>
              </div>
              <div class="my-4">
                ...
                <ul class="p-0">
                  <li v-for="service in apartment.services" :key="service.id">
                    ...
                    ><span>&nbsp&nbsp{{ service.type_of_service }}</span>
                  </li>
                </ul>
              </div>
            </li>
    ...
    <div class="my-container">
      <div class="col-12">
        ...
        <form class="p-3" @submit.prevent>
          <div class="row">
            <div class="text-center mb-4">
              <h3>Compila il nostro breve modulo di contatto qui sotto!</h3>
              <h6>
                Il proprietario sarà lieto di rispondere alle tue domande e
          ...
          <div class="mb-3">
            <label for="email"
              >la tua mail<span class="text-danger">*</span></label
            >
            <input
              class="form-control"
              type="email"
          ...
          <div class="mb-3">
            <label for="text"
              >Il tuo messaggio<span class="text-danger">*</span></label
            >
            <textarea
              required
              id="message"
